...
class MyThread(Process):
    ...
    def run(self):
        ...
        self.dic[str(self.row)]=Result(self.ar, self.ref, self.fl, self.er, self.kanming, self.row)


class Result():
    def __init__(self, ar, ref, fl, er, kanming, row):
        super().__init__()
        self.row = row
        self.kanming = kanming
        self.er = er
...
def runn(sid, kanming, i, d):
    ar, ref, fl, er = SpiderMain(sid, kanming).craw(root_url, 2)
    d[str(i)]=Result(ar, ref, fl, er, kanming, i)
    print(d)
...
if __name__ == "__main__":
    ...
    for i in range(1, nrows):
        ...
        # t = Process(target=runn,args=(sid, kanming, i, d))
        # t.row = i
        threads.append(t)
        if i % threadnum == 0 or i == nrows - 1:
            ...
            for t in threads:
                rst=d[str(t.row)]
                ar, ref, fl, er, kan, row = rst.ar, rst.ref, rst.fl, rst.er, rst.kanming, rst.row
                csv = open('res.csv', 'a')
